...
export default function EvolutionClient({ versions }: { versions: PortfolioVersion[] }) {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {versions.map((ver, i) => (
            ...
            >
              ...
              {ver.id === 'v4' && (
                <a href="../v4-react-design-system/" className="inline-block mt-4 text-xs font-semibold text-accent-amber hover:text-accent-amber-hover transition-colors">
                  Visit V4 →
                </a>
              )}
              {ver.id === 'v6' && (
                <a href="../v6-ai-native/" className="inline-block mt-4 text-xs font-semibold text-accent-amber hover:text-accent-amber-hover transition-colors">
                  Preview V6 →
                </a>
              )}
            </motion.div>
